...
C
     ...
     2,(CORGCX(1)+CORGCX(2)+CORGCX(3)+CORGCX(4)))/0.55
      ...
      VOLA(L,NY,NX)=AMAX1(0.0,POROS(L,NY,NX)*VOLX(L,NY,NX))
      VOLAI(L,NY,NX)=VOLA(L,NY,NX)
      VOLAH(L,NY,NX)=AMAX1(0.0,FHOL(L,NY,NX)*VOLTI(L,NY,NX))
C
...
C
     ...
     2,POROS(L,NY,NX)-THW(L,NY,NX)))
      ...
      ENDIF
      VOLW(L,NY,NX)=AMAX1(0.0,THETW(L,NY,NX)*VOLX(L,NY,NX))
      VOLWX(L,NY,NX)=VOLW(L,NY,NX)
      VOLWH(L,NY,NX)=AMAX1(0.0,THETW(L,NY,NX)*VOLAH(L,NY,NX))
      VOLI(L,NY,NX)=AMAX1(0.0,THETI(L,NY,NX)*VOLX(L,NY,NX))
      VOLIH(L,NY,NX)=AMAX1(0.0,THETI(L,NY,NX)*VOLAH(L,NY,NX))

...
C
     ...
     9,FLWLY(3,JD,JV,JH)
      DIMENSION FLWHL(3,JD,JV,JH),HFLWL(3,JD,JV,JH),FLWHLY(3,JD,JV,JH)
...
C
     ...
     2-THETW1)/POROS(N3,N2,N1))+1))
      KL=MAX(1,MIN(100,INT(100.0*(POROS(N6,N5,N4)
    !  2-AMIN1(THETS(N6,N5,N4),THETWL))/POROS(N6,N5,N4))+1))   ! origional code
...
C
     ...
     2,VOLWX1(N3,N2,N1)/VOLY(N3,N2,N1)))
      THETWL=THETAL
      K1=MAX(1,MIN(100,INT(100.0*(POROS(N3,N2,N1)
    !  2-AMIN1(THETS(N3,N2,N1),THETW1))/POROS(N3,N2,N1))+1))     origional code
...
9568  CONTINUE
      ...
      ENDIF
C     Uncomment 
C      IF(L.EQ.5.AND.DPTH(L,NY,NX).LT.TILEDEP(NY,NX).AND.IFLGD.EQ.0)THEN
C      WRITE(*,9567)'IFLGD',I,J,M,NX,NY,L,IFLGD,IFLGD,PSISM1(L,NY,NX)
C     1,TILEDEP(NY,NX),DPTH(L,NY,NX)
C     2,0.0098*(DPTH(L,NY,NX)-TILEDEP(NY,NX))
C     2,PSISE(L,NY,NX)+0.0098*(DPTH(L,NY,NX)-TILEDEP(NY,NX)) 
C     2,DTBLYX,RCHGFT,DTBLY(NY,NX)
C      ENDIF 
C
...
C     3,VOLAH1(L,NY,NX),VOLWH1(L,NY,NX),VOLIH1(L,NY,NX),CDPTH(L,NY,NX)
C     4,DLYR(3,L,NY,NX),DPTHH,THETX,DPTHA(NY,NX),RCHGFT,TILEDEP(NY,NX)
C      ENDIF 
C
...
C     2,XN,FLWL(N,M6,M5,M4),FLWT,PSISWT
C     3,HCND(N,1,N3,N2,N1),AREA(N,N3,N2,N1),AREAUD(N3,N2,N1)
C     3,TILEDEN(N2,N1),TILECO(N2,N1),TILEDEP(N2,N1)
...
C     3,0.0098*AMAX1(0.0,DPTH(N3,N2,N1)-DPTHT(N2,N1)),PSISWD
C      ENDIF
      ENDIF
C
...
C
     ...
     2.AND.VOLAH1(N3,N2,N1).GT.ZEROS2(N2,N1))THEN
      PSISWD=XN*0.005*SLOPE(N,N2,N1)*DLYR(N,N3,N2,N1)
     2*(1.0-DTBLG(N2,N1))
C      PSISWTH=-0.03*PSISO(N3,N2,N1)
C     2+0.0098*(DPTHH-DTBLY(N2,N1))
C     3-0.0098*AMAX1(0.0,DPTHH-DPTHT(N2,N1))
      PSISWTH=-0.03*PSISO(N3,N2,N1)
     2+0.0098*(DPTHH-TILEDEP(N2,N1))
     3-0.0098*AMAX1(0.0,DPTHH-DPTHT(N2,N1))
      IF(PSISWTH.LT.0.0)PSISWTH=PSISWTH-PSISWD
C     Changed by zewei to meet the requirement for tile drainage
C      FLWTH=PSISWTH*CNDH1(N3,N2,N1)*AREA(N,N3,N2,N1)  
C     2*(1.0-AREAUD(N3,N2,N1))/(RCHGFU+1.0)*RCHGFT*XNPH 
      FLWTH=PSISWTH*CNDH1(N3,N2,N1)*AREA(N,N3,N2,N1)  
     2*(1.0-AREAUD(N3,N2,N1))/(TILEDEN(N2,N1)+1.0)*TILECO(N2,N1)*XNPH 

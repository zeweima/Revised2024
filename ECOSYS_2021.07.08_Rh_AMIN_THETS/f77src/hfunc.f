...
224   FORMAT(A8,3I6,5E12.4,3I6,3E12.4,2I6,1E12.4,2I6,2E12.4)
     ...
     2+NNOD(NZ,NY,NX)/FNOD(NZ,NY,NX)+XTLI(NZ,NY,NX)
     3.OR.(ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).EQ.1
     3.AND.IPTYP(NZ,NY,NX).EQ.2))THEN
...
C
     ...
     3.OR.(ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).EQ.1
     2.AND.IPTYP(NZ,NY,NX).EQ.2))THEN
...
C     WFNSP=water stress effect on phenology
C
...
C    4,PSTG(NB,NZ,NY,NX),GROUP(NB,NZ,NY,NX)
C    4,PSTGI(NB,NZ,NY,NX),XPPD(NZ,NY,NX)
333   FORMAT(A8,8I6,20E12.4)
C     ENDIF
...
C
     ...
     3.OR.IPTYP(NZ,NY,NX).NE.2)
     3.OR.IDAY(2,NB,NZ,NY,NX).NE.0)
     2.OR.(ALAT(NY,NX).GT.0.0.AND.I.EQ.173)
     3.OR.(ALAT(NY,NX).LT.0.0.AND.I.EQ.355))THEN
      VRNF(NB,NZ,NY,NX)=0.0
      ENDIF
...
C
     ...
     3.AND.ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).EQ.1
     2.AND.IPTYP(NZ,NY,NX).EQ.2))THEN
